...
// beginEdit configures the shared editInput to edit `value` with the
// given placeholder + masked flag, focuses it (with cursor at end), and
// returns the blink command that must be propagated to keep the caret
// alive.
func (m *Model) beginEdit(value, placeholder string, masked bool) tea.Cmd {
...
// endEdit captures the current input value into formFields[index] (when
// index >= 0) and blurs the input, matching the existing formFields
// contract used by save paths.
func (m *Model) endEdit(index int) string {
	v := m.editInput.Value()
	if index >= 0 && index < len(m.formFields) {
		m.formFields[index] = v
	}
	m.editInput.Blur()
	return v
}
